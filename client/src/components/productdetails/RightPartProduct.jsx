    ...
    >
      ...
      {productList.length === 0 ? (
        ...
        productList.slice(0, 6).map((item) => (
          <Link
            key={item.id}
            className="flex w-full gap-x-2  xl:gap-x-4  py-5 border-b hover:bg-gray-200"
          >
            <Link
            ...
            <div className="w-[70%]">
              <Link
                to={`/productdetail/${item?.product?.name
                  ?.toLowerCase()
                  .replace(/\s+/g, "-")}/${item?.product?._id}`}
                className="text-[12px] xl:text-sm font-normal block -pt-1 text-texthead Truncate h-14  overflow-hidden"
              >
                {item?.product?.name}
              </Link>
