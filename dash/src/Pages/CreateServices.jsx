...
const Services = () => {
  ...
  const handleCreateOrEdit = async (values) => {
    ...
    // Handle icon with default value
    formData.append(
      "icon",
      values.icon && values.icon.trim() !== ""
        ? values.icon
        : "<i class='fa-solid fa-tools'></i>" // Default icon
    );
  
    // Handle file upload
    if (fileList.length > 0 && fileList[0].originFileObj) {
      formData.append("photo", fileList[0].originFileObj);
    } else if (editingService?.photo) {
    ...
    try {
      ...
      console.error("Error saving service:", error); // Log error for debugging
      message.error(
